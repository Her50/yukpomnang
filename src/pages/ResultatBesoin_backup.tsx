...
// Fonction utilitaire pour extraire la valeur d'un champ de service - VERSION CORRIGÉE
const getServiceFieldValue = (field: any): string => {
  ...
  // Cas 2: Objet complexe (nouveaux services) - STRUCTURE CORRIGÉE
  if (field && typeof field === 'object') {
    // Vérifier si c'est un objet avec la structure {valeur, type_donnee, origine_champs}
    ...
    // Vérifier si c'est un objet avec d'autres propriétés
    if (Object.keys(field).length > 0) {
      // Essayer de trouver une valeur dans les propriétés communes
...
// Fonction utilitaire pour déboguer les champs de contact d'un service
const debugServiceContacts = (service: any) => {
  console.log('🔍 [DEBUG] Structure complète du service:', service);
  console.log('🔍 [DEBUG] Service.data:', service.data);
  
  if (service.data) {
    console.log('🔍 [DEBUG] Champs de contact dans service.data:');
    console.log('  - telephone:', service.data.telephone);
    console.log('  - email:', service.data.email);
    console.log('  - website:', service.data.website);
    console.log('  - whatsapp:', service.data.whatsapp);
    
    // Tester la fonction getServiceFieldValue
    console.log('🔍 [DEBUG] Test getServiceFieldValue:');
    console.log('  - telephone extrait:', getServiceFieldValue(service.data.telephone));
    console.log('  - email extrait:', getServiceFieldValue(service.data.email));
    console.log('  - website extrait:', getServiceFieldValue(service.data.website));
    console.log('  - whatsapp extrait:', getServiceFieldValue(service.data.whatsapp));
  }
};
                ...
                <CardContent className="pt-0">
                  ...
                  <div className="space-y-2">
                      ...
                      {(!getServiceFieldValue(service.data?.telephone) || getServiceFieldValue(service.data?.telephone) === 'Non spécifié') &&
                       (!getServiceFieldValue(service.data?.email) || getServiceFieldValue(service.data?.email) === 'Non spécifié') &&
                       (!getServiceFieldValue(service.data?.website) || getServiceFieldValue(service.data?.website) === 'Non spécifié') && (
                        <div className="col-span-2 p-3 bg-gray-50 border border-gray-200 rounded-xl text-center">
                          <p className="text-sm text-gray-500">Aucun contact supplémentaire disponible</p>
                          <p className="text-xs text-gray-400 mt-1">Utilisez le bouton "Contacter le prestataire" ci-dessus</p>
                        </div>
                      )}
                    </div>
                  </div>
