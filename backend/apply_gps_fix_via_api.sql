-- Application des corrections GPS via l'API du backend
-- ===================================================
...
DROP FUNCTION IF EXISTS extract_gps_coordinates(jsonb);
DROP FUNCTION IF EXISTS extract_gps_from_json(jsonb);
DROP FUNCTION IF EXISTS fast_gps_search_with_user_fallback(text, text, integer);
DROP FUNCTION IF EXISTS fast_text_gps_search_with_user_fallback(text, text, integer);
DROP FUNCTION IF EXISTS get_user_gps(integer);
DROP FUNCTION IF EXISTS calculate_gps_distance_km(double precision, double precision, double precision, double precision);
DROP FUNCTION IF EXISTS calculate_intelligent_radius(double precision);

-- 2. Créer la fonction extract_gps_from_json corrigée
...
BEGIN
    ...
    ELSIF jsonb_typeof(gps_data) = 'string' THEN
        ...
        lat_lng := gps_data::text;
        -- Enlever les guillemets
        lat_lng := trim(both '"' from lat_lng);
        
        IF lat_lng ~ '^-?\d+\.?\d*,-?\d+\.?\d*$' THEN
            RETURN lat_lng;
        END IF;
    END IF;
...
-- 3. Créer la fonction calculate_gps_distance_km
...
-- 4. Créer la fonction calculate_intelligent_radius
...
-- 5. Créer la fonction get_user_gps
...
-- 6. Créer la fonction search_services_gps_final principale
...
BEGIN
    ...
    IF gps_zone IS NOT NULL AND gps_zone != '' THEN
        ...
        IF array_length(gps_parts, 1) > 0 THEN
            ...
            -- Construire la requête de recherche avec filtrage GPS
            search_query := format('
                SELECT 
                    s.id,
                    ...
                    GREATEST(
                        CASE 
                            WHEN s.data->>''titre_service'' ILIKE %L THEN 100
                            WHEN s.data->>''description'' ILIKE %L THEN 50
                            ELSE 10
                        END,
                        CASE 
                            WHEN s.data->>''category'' ILIKE %L THEN 30
                            ELSE 0
                        END
                    ) as score,
                    calculate_gps_distance_km(%L, %L, 
                ...
                FROM services s
                WHERE 
                    s.data->>''titre_service'' ILIKE %L
                    OR s.data->>''description'' ILIKE %L
                    OR s.data->>''category'' ILIKE %L
                    AND s.data->''gps_fixe'' IS NOT NULL
                    AND extract_gps_from_json(s.data->''gps_fixe'') IS NOT NULL
                    AND calculate_gps_distance_km(%L, %L,
                        split_part(extract_gps_from_json(s.data->''gps_fixe''), '','', 1)::double precision,
                        split_part(extract_gps_from_json(s.data->''gps_fixe''), '','', 2)::double precision
                    ) <= %L
...
-- 7. Créer les fonctions de fallback
...
-- 8. Vérifier que les fonctions sont créées
SELECT 
    routine_name,
    routine_type,
    'OK' as status
