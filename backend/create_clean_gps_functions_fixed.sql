-- Création des fonctions GPS propres et bien définies (CORRIGÉ)
-- =============================================================
    ...
    IF gps_zone IS NOT NULL AND gps_zone != '' THEN
        ...
        IF array_length(gps_parts, 1) > 0 THEN
            ...
            SELECT 
                ...
                GREATEST(
                    CASE 
                        WHEN s.data->>'titre_service' ILIKE '%' || search_query || '%' THEN 100
                        WHEN s.data->>'description' ILIKE '%' || search_query || '%' THEN 50
                        ELSE 10
                    END,
                    CASE 
                        WHEN s.data->>'category' ILIKE '%' || search_query || '%' THEN 30
                        ELSE 0
                    END
                ) as score,
...
BEGIN
    RETURN QUERY SELECT * FROM search_services_gps_final(search_query, gps_zone, radius_km);
END;
$$ LANGUAGE plpgsql STABLE;
