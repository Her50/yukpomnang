-- Correction du type de retour de la fonction search_images_by_metadata
-- Base de données: yukpo_db

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS search_images_by_metadata(TEXT, INTEGER);
...
BEGIN
    ...
    IF query_width IS NOT NULL AND query_height IS NOT NULL AND query_height > 0 THEN
        query_ratio := query_width::NUMERIC / query_height::NUMERIC;
    ELSE
        query_ratio := 1.0;
    END IF;
        ...
        (
            ...
            CASE
                WHEN m.image_metadata->>'width' IS NOT NULL AND m.image_metadata->>'height' IS NOT NULL THEN
                    CASE
                        WHEN ABS((m.image_metadata->>'width')::INTEGER::NUMERIC / (m.image_metadata->>'height')::INTEGER::NUMERIC - query_ratio) < 0.1 THEN 0.3
                        WHEN ABS((m.image_metadata->>'width')::INTEGER::NUMERIC / (m.image_metadata->>'height')::INTEGER::NUMERIC - query_ratio) < 0.3 THEN 0.1
                        ELSE 0.0
                    END
                ELSE 0.0
            END +
            ...
            CASE
                WHEN m.image_metadata->>'file_size' IS NOT NULL AND query_metadata_json->>'file_size' IS NOT NULL THEN
                    CASE
                        WHEN ABS((m.image_metadata->>'file_size')::INTEGER - (query_metadata_json->>'file_size')::INTEGER)::NUMERIC / (query_metadata_json->>'file_size')::INTEGER::NUMERIC < 0.2 THEN 0.2
                        ELSE 0.0
                    END
                ELSE 0.0
            END
...
SELECT 'Fonction search_images_by_metadata corrigée avec type NUMERIC' as status;
