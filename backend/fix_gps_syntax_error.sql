-- CORRECTION SYNTAXE JSON - FONCTION GPS FINALE
-- =============================================
...
-- 2. Créer la fonction corrigée avec syntaxe JSON appropriée
                        ...
                        WHEN s.data->>'gps_fixe' IS NOT NULL AND s.data->>'gps_fixe' != '' AND s.data->>'gps_fixe' != 'false' THEN
                            CASE 
                                ...
                                WHEN s.data->>'gps_fixe' ~ '^\{.*"lat".*"lon".*\}.*$' THEN
                                    (s.data->'gps_fixe'->>'lat') || ',' || (s.data->'gps_fixe'->>'lon')
                            ...
                            CASE 
                                ...
                                WHEN s.data->>'gps_fixe' ~ '^\{.*"lat".*"lon".*\}.*$' THEN
                                    calculate_gps_distance_km(lat, lng,
                                        (s.data->'gps_fixe'->>'lat')::double precision,
                                        (s.data->'gps_fixe'->>'lon')::double precision
                                    )
                                -- Format simple: "4.05,9.71"
                                WHEN s.data->>'gps_fixe' ~ '^-?\d+\.?\d*,-?\d+\.?\d*$' THEN
                                    calculate_gps_distance_km(lat, lng,
